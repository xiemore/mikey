<template>
  <div class="firstContainer">
    <div class="secondContainer">
      ...
      <el-table v-if="showType === 'create'" :data="orders">
        <el-table-column
            label="商品名称">
...
export default {
  ...
  methods: {
    init_orders_now(){
      postRequest("/order/selectOrdersCreated", JSON.stringify({page: this.page, size:this.size})).then(resp => {
    ...
    init_orders_done(){
      postRequest("/order/selectOrdersDone", JSON.stringify({page: this.page, size:this.size})).then(resp => {
        if (resp.data["code"] === 200){
          this.orders = resp.data["data"]["orders"]
        } else {
          ElMessage.info("权限不足")
    ...
    init_total_created(){
      postRequest("/order/selectOrdersCreatedTotal").then(resp => {
        if (resp.data["code"] === 200) {
    ...
    init_total_done(){
      postRequest("/order/selectOrdersDoneTotal").then(resp => {
        if (resp.data["code"] === 200) {
          this.total = resp.data["data"]["total"]
        } else {
          ElMessage.error("未知错误")
    ...
    cancel(good_id, order_id){
      postRequest("/order/cancelOrder", JSON.stringify({"order_id": order_id, "good_id": good_id})).then(resp => {
        if (resp.data["code"] === 200){
          ElMessage.info("操作成功")
        } else {
          ElMessage.info("权限不足")
    ...
    complete(good_id, order_id){
      postRequest("/order/completeOrder", JSON.stringify({"order_id": order_id, "good_id": good_id})).then(resp => {
        if (resp.data["code"] === 200){
          ElMessage.info("操作成功")
        } else {
          ElMessage.info("权限不足")
